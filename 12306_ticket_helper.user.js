...
var utility = {
	...
	delayInvoke: function (target, callback, timeout) {
		...
		if (timeout <= 0) {
			...
			e.html(str + " 秒后再�ader.removeClass("steps stepsok");
...
function initLogin() {
	...
	if (tw.utility.allPassengers) {�!...").removeClass("fish_running").addClass("fish_clock");
			setTimeout(function () {
...
fter("<span id='status_" + this + "'>正在查询...</span>"); });
